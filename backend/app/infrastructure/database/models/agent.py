...
class TurnRow(Base):
    ...
    )  # Turn 状态机：pending/running/committed/failed/cancelled
...
class RunStepRow(Base):
    ...
    )  # 步骤序号（与 run_id 联合唯一，保证轨迹有序）
    kind: Mapped[str] = mapped_column(
        String(32), nullable=False
    )  # 步骤类型：reasoning/tool_call/observation/final
    call_id: Mapped[UUID | None] = mapped_column(
        nullable=True, index=True
    )  # 工具调用 ID，用于把 tool_call 与 observation 配对
...
class AgentRunCheckpointRow(Base):
    """AgentRun 轻量检查点：每次成功 Observation 后写入，供失败后续跑。"""

    __tablename__ = "agent_run_checkpoints"
    __table_args__ = (
        UniqueConstraint("run_id", "step_index", name="uq_agent_run_checkpoints_run_step"),
    )
    ...
    turn_id: Mapped[UUID] = mapped_column(nullable=False)  # 所属 Turn
    user_id: Mapped[UUID] = mapped_column(
        ForeignKey("users.id"), nullable=False, index=True
    )  # 归属用户
    thread_id: Mapped[UUID] = mapped_column(nullable=False)  # 所属线程
    step_index: Mapped[int] = mapped_column(Integer, nullable=False)  # 下一轮 Reason 步序号
    current_input: Mapped[str] = mapped_column(Text, nullable=False)  # 本轮用户原文
    interactions: Mapped[list[Any]] = mapped_column(JSONB, nullable=False)  # 工具轨迹快照
    discovered_tool_names: Mapped[list[Any]] = mapped_column(
