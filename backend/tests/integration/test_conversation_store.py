...
async def test_start_and_commit_turn(
    ...
    thread = await store.get_or_create_thread(user_id=user_id, thread_id=None)
    started = await store.start_turn(user_id=user_id, thread_id=thread.id, content="hello")
...
async def test_fail_turn_keeps_user_message_without_assistant(
    ...
    store = SqlAlchemyConversationStore(sessions, clock)
    thread = await store.get_or_create_thread(user_id=user_id, thread_id=None)
    started = await store.start_turn(user_id=user_id, thread_id=thread.id, content="oops")
...
async def test_cancel_turn(
    ...
    store = SqlAlchemyConversationStore(sessions, clock)
    thread = await store.get_or_create_thread(user_id=user_id, thread_id=None)
    started = await store.start_turn(user_id=user_id, thread_id=thread.id, content="stop")
...
async def test_thread_belongs_to_user(
    ...
    with pytest.raises(ForbiddenError):
        await store.get_or_create_thread(user_id=other, thread_id=thread.id)
